...
mesh_t mesh_load(char* path) {
  ...
  for (int i = 0; i < mesh->mNumVertices; i++) {
    ...
    result.vertices[i] = (vertex_t){{position.x, position.y, position.z}, {normal.x, normal.y, normal.z}, {1.0, 1.0, 1.0, 1.0}, {tex_coords.x, tex_coords.y}};
  }
  ...
  for (int i = 0; i < mesh->mNumFaces; i++) {
    ...
    for (int j = 0; j < 3; j++)
      result.indices[i * 3 + j] = face.mIndices[j];
  }
...
void mesh_setup(mesh_t* mesh) {
  ...
  vao_attrib(&mesh->vao, 3, GL_FLOAT, GL_FALSE, offsetof(vertex_t, color));
